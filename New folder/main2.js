...
const students = [
  ...
  function getSubjectsStatistics(students) {
     const average = students.map(students => (students.math+students.history + students.physics)/3)
     return average
     return students.map(({name,lastname, average}) => [name,lastname,average])
    }
  
  function getHighAchievers(students, gradeThreshold) {
    return students.filter(students => students.average>gradeThreshold).map(students => [students.name])
  }
  
  function getMultiAchivers(students, threshold) {
    students.filter(students => 
        if(students.math>threshold || students.history>threshold || students.physics>threshold){
          return students
        })
